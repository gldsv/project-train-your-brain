...
class Postproc():
    ...
    def __init__(self, text, predict_data, preds):
    ...
    def aggregation_predicator(self):
        ...
        #peut-être rajouter un IF ici pour quand il n'y aura pas les targets
...
#    predict_data = pd.read_json(f'{url}/train_data/label_train_data_tokenized.json',orient='records',)
#    preds = '/Users/laurene.merite/code/laurene-merite-wagon/project-train-your-brain/test/test8.npy'
#    tmp = Postproc(text=text, predict_data=predict_data,preds=preds)
#    #print(tmp.preds[1])
#    tmp = tmp.aggregation_predicator()
#    tmp = tmp.postprepoc_df()
#    tmp.to_csv(f"{url}/postprocessed_data_{text.split('/')[-1].split('.')[0].split('_')[1]}_true.csv")
